...
basis_MLP <- function(input,output,learning.rate=0.0001,basis,step = 20)
{
  # input dimension
  m <- dim(input)[2]
  ...
  b_2 <- rep(list(matrix(NA, nrow = n,ncol = n_bas)), 
             times = N)
  # iterate training
  for(iter in 1:step)
  {
    ...
    # initialize gradient
    grad_b2 <- rep(list(matrix(NA, nrow = n,ncol = n_bas)), 
                   times = N)
    grad_a2 <- matrix(NA,nrow = n_bas, ncol = N)
    
    grad_phi <- rep(list(matrix(NA,nrow = n, ncol = n_bas)),
                    times = N)
    grad_wt <- rep(list(matrix(NA,nrow = n_bas, ncol = m)),
                   times = N)
    
    for(j in 1:N)
    {
      grad_b2[[j]] <- (b_2[[i]] %*% wt - output[j,]) %*% t(wt)/N
      grad_a2[,j] <- t(grad_b2[[j]] * basis) %*% matrix(1,nrow = n)
      grad_phi[[j]] <- grad_b2[[j]] * (matrix(1,nrow = n)%*%t(a_2[,j]))
      grad_wt[[j]] <- (grad_a2[,j] * a_2[,j]) %*% 
        matrix(input[j,],nrow = 1)
    }
  ...
  # return final model structure
  return(list(W_1 = W_1, output_dimension = n ,wt = wt, basis = basis))
}


# ============================ Prediction part ========================= #
bs_MLP_predict <- function(test,n ,W_1, wt,basis)
{
